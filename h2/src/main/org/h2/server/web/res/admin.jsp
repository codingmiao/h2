...
<body style="margin: 20px">
	...
	<form name="admin" method="post"
		action="adminSave.do?jsessionid=${sessionId}">
		<h3>${text.adminAllow}</h3>
		<p>
		...
		<p>${text.adminRestart}</p>
	</form>
	<hr />
	<p>
	<form name="translate" method="post"
		action="adminStartTranslate.do?jsessionid=${sessionId}">
		<input type="submit" class="button"
			value="${text.adminTranslateStart}" />
	</form>
	</p>
	<p>${text.adminTranslateHelp}</p>
	<hr />
	<h3>${text.adminSessions}</h3>
	<table>
		<tr>
			<th>${text.admin.ip}</th>
		...
		<c:forEach var="item" items="sessions">
			<tr>
				...
				<td>${item.lastQuery}</td>
			</tr>
		</c:forEach>
	</table>
	<br />
	<form name="shutdown" method="post"
		action="adminShutdown.do?jsessionid=${sessionId}">
